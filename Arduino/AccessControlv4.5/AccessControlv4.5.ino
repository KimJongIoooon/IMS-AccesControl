...
byte storedHours[4]; //stores hours read from EEPROM

byte readCard[4];   // Stores scanned ID read from RFID Module
byte readHours[4];  // not used yet will prob store hours read from serial mon.


byte hours[4] = {07, 30, 22, 30};
  ...
  if (programMode) {
    ...
    if ( isMaster(readCard)) {    // If scanned card's ID matches Master Card's ID - enter program mode
      ...
      if ( findID(readCard) ) { // If not, see if the card is in the EEPROM
        ...
        //        if(startHour <  hour(t) && startMinutes <  minute(t)){
...
void readID( uint8_t number ) {
  ...
  for ( uint8_t i = 0; i < 4; i++ ) {     // Loop 4 times to get the 4 Bytes
    ...
    storedHours[i] = EEPROM.read(start + i + 4); //Assign hours from EEPROM to array
  }
}

///////////////////////////////////////// Add ID to EEPROM   ///////////////////////////////////
void writeID( byte a[],  byte hours[]) {
  if ( !findID( a ) ) {     // Before we write to the EEPROM, check to see if we have seen this card before!
    ...
    failedWrite();
    Serial.println(F("#Failed to add! There is something wrong with ID or bad EEPROM%"));
  }
}
...
void deleteID( byte a[] ) {
  if ( !findID( a ) ) {     // Before we delete from the EEPROM, check to see if we have this card!
    ...
    for ( uint8_t k = 0; k < 8; k++ ) {         // Shifting loop
...
bool findID( byte find[] ) {
  uint8_t count = EEPROM.read(0);     // Read the first Byte of EEPROM that counts number of ids
...
void serialCommunicationIn() {
  //reads the incomming messages, if there is an command inside it executes it.
  if (Serial.available() == SERIAL_RX_BUFFER_SIZE) {
    //SERIAL_RX_BUFFER_SIZE zit vol
  }
  if (Serial.available() > 0) {
    char readChar = (char) Serial.read();
    if (readChar == '#') {
      //starts new message if # is found
      //# marks the start of a message
      message = "";
      messageStarted = true;
    } else if (messageStarted) {
      if (readChar == '%') {
        //Send message if end of message is reached
        //% marks the end of a message
        //executes the command in the message
        executeSerialCommand(message);
        message = "";
        messageStarted = false;
      } else if (message.length() > 255) { //too long
        messageStarted = false;
        message = "";
      } else {
...
void executeSerialCommand(String command) {
  ...
  if (command.startsWith("DELETE_CARD:")) {
    deleteCard(id);
  }
